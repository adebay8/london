...
- **Fresh DB / rebuild:** the flat store lives in MongoDB (Atlas). `npx tsx scripts/flat-search-seed.ts` bootstraps it from the committed baseline `data/flat-search-seed.json` (skips if listings already exist; `--force` to overwrite). Running the search then advances the live DB from there. (The legacy `london.db` SQLite file is kept only as a pre-migration backup.)
